...
int loadPrivDER(char *[], int) {
  ...
  if (dutils::hexStr(der_synth) != dutils::hexStr(der_loaded))
    return 1;
  return 0;
}
...
int importBundlePEM(char *argv[], int argc) {
  if (importPrivPEM(argv, argc) || importPubPEM(argv, argc))
    return 1;
  return 0;
}
...
int loadPrivPEM(char *[], int) {
  ...
  if (dutils::hexStr(keyc_synth.getPrivDER()) !=
      dutils::hexStr(keyc_loaded.getPrivDER()))
    return 1;
  return 0;
}
...
int loadPubPEM(char *[], int) {
  ...
  if (dutils::hexStr(keyc_synth.getPubDER()) !=
      dutils::hexStr(keyc_loaded.getPubDER()))
    return 1;
...
int Encrypt_test(char *[], int) {
  ...
  if (dutils::hexStr(payload) == dutils::hexStr(res) || res.size() == 0)
    return 1;
...
int Decrypt_test(char *[], int) {
  ...
  if (dutils::hexStr(payload) == dutils::hexStr(res_decrypted))
    return 0;
...
int hashTest(char *[], int) {
  dutils::dbuffer payload("Hewwo, I am secret ^.^");
  dutils::dbuffer hashed_payload =
      tancrypt::hash(payload, tancrypt::hashAlg::SHA256);
...
int signTest(char *[], int) {
  ...
  dutils::dbuffer signature =
      tancrypt::RSA::sign(key, payload, tancrypt::hashAlg::SHA256);
...
int verifyTest(char *[], int) {
  ...
  std::cout << dutils::hexStr(signature) << std::endl;
  bool res =
      tancrypt::RSA::verify(key, signature, payload, tancrypt::hashAlg::SHA256);
...
int AESKEY_init2Test(char *[], int) {
  using namespace tancrypt;
  dutils::dbuffer my_keydata("Hewwo, I am secret >.<");
  tancrypt::AES::keyc key_variant2(my_keydata, AES::Type::CBC256,
                                   hashAlg::SHA256);
...
int AesDecryptV2(char *[], int) {
  ...
  std::cout << "Decrypted(hex): " << dutils::hexStr(dec_buffer) << std::endl;
  std::cout << "Decrypted: " << dec_buffer.data() << std::endl;
  if (dutils::hexStr(dec_buffer) != dutils::hexStr(payload))
    return 1;
...
int getNonce_test(char *[], int) {
  ...
  dutils::dbuffer nonce_out =
      tancrypt::AES::getNonce(enc_buffer, AES::Type::CBC256);
